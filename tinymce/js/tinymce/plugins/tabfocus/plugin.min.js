...
} else for (l=h-1; l>=0; l--) if (g(j[l])) return j[l]; return null;
} let h, j, k, l; if (!(9!==b.keyCode||b.ctrlKey||b.altKey||b.metaKey||b.isDefaultPrevented())&&(k=c.explode(a.getParam('tab_focus', a.getParam('tabfocus_elements', ':prev,:next'))), 1==k.length&&(k[1]=k[0], k[0]=':prev'), j=b.shiftKey?':prev'==k[0]?g(-1):i.get(k[0]):':next'==k[1]?g(1):i.get(k[1]))) {
const m=d.get(j.id||j.name); j.id&&m?m.focus():e.setTimeout(function() {
