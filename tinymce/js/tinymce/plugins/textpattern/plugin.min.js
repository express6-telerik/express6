...
for (let c=0; c<a.length; c++) if (0===b.indexOf(a[c].start)&&(!a[c].end||b.lastIndexOf(a[c].end)===b.length-a[c].end.length)) return a[c];
}, c=function(a, b, c, d) {
const e=b.substr(c-a.end.length-d, a.end.length); return e===a.end;
}, d=function(a, b, c) {
return a-b-c.end.length-c.start.length>0;
}, e=function(b, e, f, g) {
let h, i, j=a(b); for (i=0; i<j.length; i++) if (h=j[i], void 0!==h.end&&c(h, e, f, g)&&d(f, g, h)) return h;
...
let f, g, h, i, j, k, l, m, n, o, p; if (f=a.selection, g=a.dom, f.isCollapsed()&&(h=f.getRng(!0), i=h.startContainer, j=h.startOffset, l=i.data, o=e===!0?1:0, 3==i.nodeType&&(n=c.findEndPattern(b, l, j, o), void 0!==n&&(k=Math.max(0, j-o), k=l.lastIndexOf(n.start, k-n.end.length-1), k!==-1&&(m=g.createRng(), m.setStart(i, k), m.setEnd(i, j-o), n=c.findPattern(b, m.toString()), n&&n.end&&!(i.data.length<=n.start.length+n.end.length)))))) {
return p=a.formatter.get(n.format), p&&p[0].inline?(a.undoManager.transact(function() {
i=d(i, n, j, k, o), a.formatter.apply(n.format, {}, i);
}), i):void 0;
}
}, f=function(d, e) {
let f, g, h, i, j, k, l, m, n, o, p; if (f=d.selection, g=d.dom, f.isCollapsed()&&(l=g.getParent(f.getStart(), 'p'))) {
...
function c(a, c) {
let d, e; e=b.applyInlineFormat(a, c, !1), e&&(d=a.dom.createRng(), d.setStart(e, e.data.length), d.setEnd(e, e.data.length), a.selection.setRng(d)), b.applyBlockFormat(a, c);
} function d(a, c) {
let d, e, f, g, h; d=b.applyInlineFormat(a, c, !0), d&&(h=a.dom, e=d.data.slice(-1), /[\u00a0 ]/.test(e)&&(d.deleteData(d.data.length-1, 1), f=h.doc.createTextNode(e), d.nextSibling?h.insertAfter(f, d.nextSibling):d.parentNode.appendChild(f), g=h.createRng(), g.setStart(f, 1), g.setEnd(f, 1), a.selection.setRng(g)));
} let e=function(a, b, c) {
for (let d=0; d<a.length; d++) if (c(a[d], b)) return !0;
}, f=function(b, c) {
...
let a=[{ start: '*', end: '*', format: 'italic' }, { start: '**', end: '**', format: 'bold' }, { start: '#', format: 'h1' }, { start: '##', format: 'h2' }, { start: '###', format: 'h3' }, { start: '####', format: 'h4' }, { start: '#####', format: 'h5' }, { start: '######', format: 'h6' }, { start: '1. ', cmd: 'InsertOrderedList' }, { start: '* ', cmd: 'InsertUnorderedList' }, { start: '- ', cmd: 'InsertUnorderedList' }], b=function(b) {
...
let d=f.getPatterns(a.settings), g=[',', '.', ';', ':', '!', '?'], h=[32]; a.on('keydown', function(b) {
